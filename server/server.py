...
import threading
import uuid
from rsa.rsa_utils import generate_rsa_keypair, serialize_public_key, deserialize_public_key
...
def start_server():
    ...
    print("[SERVER] Сервер запущен на порту 12345")

    # Генерация ключей RSA для сервера
    server_private_key, server_public_key = generate_rsa_keypair()
    print("[SERVER] Сгенерированы ключи RSA для сервера")

    while True:
        # Ожидание подключения клиента
        client_socket, addr = server.accept()
        print("[SERVER] Принято соединение от клиента:", addr)
        ...
        print("[SERVER] Получен публичный ключ клиента")

        # Сохранение связи между UUID клиента и его публичным ключом
        clients[client_id] = client_socket
        print("[SERVER] Связь сохранена")
        ...
        client_thread.start()
        print("[SERVER] Связь установлена")
